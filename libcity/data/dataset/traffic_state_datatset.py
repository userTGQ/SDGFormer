...
class TrafficStateDataset(AbstractDataset):
    ...
    def _generate_input_data(self, df):
        ...
        for t in range(min_t, max_t):
            ...
            if self.input_window == 12:
                c = t - 2016+self.input_window
                if c < self.input_window - 1:
                    x_t_pro = df[t + x_offsets, :, 0][:, :, np.newaxis]
                    y_t_pro = df[t + y_offsets, :, 0][:, :, np.newaxis]
                    x_t1 = df[t + x_offsets, ...]
                    y_t1 = df[t + y_offsets, ...]
                    x_t = np.concatenate((x_t1, x_t_pro), axis=2)
                    y_t = np.concatenate((y_t1, y_t_pro), axis=2)
                else:
                    x_t_pro = df[c + x_offsets, :, 0][:, :, np.newaxis]
                    y_t_pro = df[c + y_offsets, :, 0][:, :, np.newaxis]
                    x_t1 = df[t + x_offsets, ...]
                    y_t1 = df[t + y_offsets, ...]
                    x_t = np.concatenate((x_t1, x_t_pro), axis=2)
                    y_t = np.concatenate((y_t1, y_t_pro), axis=2)
            else:
                c = t - 336+self.input_window
                if c < self.input_window - 1:
                    x_t_pro = df[t + x_offsets, :, :2]
                    y_t_pro = df[t + y_offsets, :, :2]
                    x_t1 = df[t + x_offsets, ...]
                    y_t1 = df[t + y_offsets, ...]
                    x_t = np.concatenate((x_t1, x_t_pro), axis=2)
                    y_t = np.concatenate((y_t1, y_t_pro), axis=2)
                else:
                    x_t_pro = df[c + x_offsets, :, :2]
                    y_t_pro = df[c + y_offsets, :, :2]
                    x_t1 = df[t + x_offsets, ...]
                    y_t1 = df[t + y_offsets, ...]
                    x_t = np.concatenate((x_t1, x_t_pro), axis=2)
                    y_t = np.concatenate((y_t1, y_t_pro), axis=2)
    ...
    def get_data(self):
        ...
        x_train[..., -1] = self.scaler.transform(x_train[..., -1])
        y_train[..., -1] = self.scaler.transform(y_train[..., -1])
        x_val[..., -1] = self.scaler.transform(x_val[..., -1])
        y_val[..., -1] = self.scaler.transform(y_val[..., -1])
        x_test[..., -1] = self.scaler.transform(x_test[..., -1])
        y_test[..., -1] = self.scaler.transform(y_test[..., -1])
