...
def calculate_errors(y_true, y_pred):
    ...
    # Iterate over each time step (12)
    for t in range(y_true.shape[1]):
        # Get the true and predicted values for the current time step
        true_values = y_true[:, t, :,0]
        pred_values = y_pred[:, t, :,0]
        ...
        # Compute MAPE (excluding zeros)
        non_zero_mask = true_values != 0  # Mask to exclude zeros
        non_zero_true_values = true_values[non_zero_mask]
        non_zero_pred_values = pred_values[non_zero_mask]
        if len(non_zero_true_values) > 0:
            mape = np.mean(np.abs((non_zero_true_values - non_zero_pred_values) / non_zero_true_values)) * 100
        else:
            mape = 0  # Avoid division by zero if all true values are zero
        mape_list.append(mape)

    return mae_list, rmse_list, mape_list

class SDGFormerExecutor(TrafficStateExecutor):
    ...
    def evaluate(self, test_dataloader):
        ...
        with torch.no_grad():
            ...
            mae, rmse, mape = calculate_errors(y_truths, y_preds)
